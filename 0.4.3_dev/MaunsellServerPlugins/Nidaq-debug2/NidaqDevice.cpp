...
//Lockable *NidaqDevice::NidaqDriverLock;
...
void NidaqDevice::createDigitalInTask() {
	...
	if( DAQmxFailed(niError=(DAQmxBaseCreateTask("DI",&diTask))))
	{
...
void NidaqDevice::createDigitalOutTask() {
	...
	NidaqDriverLock.lock();
	if( DAQmxFailed(niError=(DAQmxBaseCreateTask("DO", &doTask))))
	{
...
bool NidaqDevice::readDI()
{
	...
	NidaqDriverLock.lock();
	if (DAQmxFailed(niError=(DAQmxBaseReadDigitalU8(diTask,1,1.0F,DAQmx_Val_GroupByChannel,r_data,1,&read,NULL))))
	{
		DAQmxBaseGetExtendedErrorInfo (errBuff, kBufferLength);
...
bool NidaqDevice::updateSwitch() {	
	...
	Data dValue(M_INTEGER, switchValue);
	
	//tell channel to read switch
	//leverPress->setValue(dValue);
...
bool NidaqDevice::attachPhysicalDevice(){                                      // attach next avaialble device to this object
	...
	//startup(); // must be outside lock, subfunctions Create etc. acq the lock
...
bool NidaqDevice::stopDeviceIO(){
	...
	if (!deviceIOrunning) {
		merror(M_IODEVICE_MESSAGE_DOMAIN, "Cannot stop IO when IO is running");
		assert(0);
		//return false;
	}
	...
	boost::mutex::scoped_lock locker(pollScheduleNodeLock);
