...
int main(int argc, char **argv)
{
    ...
    // Reset, and reopen to make sure we're still alive
    libusb_reset_device(hDevice);
    closeUSBConnection(hDevice);
    sleep(1);

    if( (hDevice = openUSBConnection(-1)) == NULL) {
        printf("Error: could not reopen USB U6 device after reset\n");
        goto done;
    }
...
int u6Reset(HANDLE hDevice) {
    ...
    sendBuff[1] = 0x99;
    sendBuff[2] = (0x01 & 0x01) | (0x02 & 0x01);  // Do hard reset (first bit soft reset; 2nd bit hard reset)
    sendBuff[3] = 0x00;
    sendSize = 4;
    normalChecksum(sendBuff, sendSize);
    uint8 checksumTotal;
        
    //Sending command to U6
    if( (sendChars = LJUSB_BulkWrite(hDevice, U6_PIPE_EP1_OUT, sendBuff, sendSize)) < sendSize) {
        printf("Error sending command data\n");
        ret = -1;
        goto cleanup;
    }
    ...
    //Reading response from U6
    if( (recChars = LJUSB_BulkRead(hDevice, U6_PIPE_EP2_IN, recBuff, recSize)) < recSize) {
        printf("Error reading command data, error code %d (if checksum is ok)\n", recBuff[3]);
        ret = -1;
        goto cleanup;
    }

    checksumTotal = normalChecksum8(recBuff, recSize);
    if(checksumTotal != recBuff[0]) {
        printf("Reset error : read buffer has bad checksum8\n");
        ret = -1;
        goto cleanup;
    }
    if (recBuff[1] != (uint8)(0x99) || recBuff[2] != (uint8)(0x00)) {
        printf("Reset error : read buffer has wrong command bytes \n");
            ret = -1;
            goto cleanup;
    }
