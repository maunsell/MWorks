...
void If::addChild(std::map<std::string, std::string> parameters,
	...
	if(act == 0) {
		throw SimpleException("Attempting to add illegal action (" + child->getTag() + ") to coditional (if) action (" + this->getTag() + ")");
	}
...
weak_ptr<State> TransitionCondition::execute() {
    //currentState->setValue(getCompactID());
	//MH 100114 - avoid many events on loop
...
weak_ptr<State> TransitionIfTimerExpired::execute() {
    //currentState->setValue(getCompactID());
	// MH 100114 - if we set this it gets written for every state Transition with multiple timers! 
